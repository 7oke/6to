...
def show_results():
    try:
        ...
        # Calcular y mostrar diferencias porcentuales en consola
        print("\nComparación de puntajes entre géneros por edad:")
        for i, age in enumerate(all_ages):
            if avg_scores_m[i] == 0 and avg_scores_f[i] == 0:
                continue
            diff = ((avg_scores_m[i] - avg_scores_f[i]) / max(avg_scores_m[i], avg_scores_f[i])) * 100
            winner = "Hombres" if avg_scores_m[i] > avg_scores_f[i] else "Mujeres"
            print(f"Edad {age}: {winner} lideran con una diferencia de {abs(diff):.2f}%")
